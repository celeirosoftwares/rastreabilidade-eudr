    ...
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="text-[#6b8f6b] text-sm mt-0.5">
          Gere relatórios de conformidade para exportação à União Europeia
        </p>
      </div>
      ...
      <div className="bg-blue-900/20 border border-blue-700/30 rounded-2xl p-4 text-sm text-blue-300">
        ...
        <p className="text-blue-400 text-xs leading-relaxed">
          A partir de dezembro de 2024, produtos como café, soja e cana-de-açúcar exportados para a UE precisam comprovar que não contribuíram para o desmatamento após 31/12/2020. 
          Os relatórios gerados aqui reúnem as informações exigidas.
        </p>
      </div>
      ...
      {lots.length === 0 ? (
        <div className="bg-[#172117] border border-[#2d3d2d] border-dashed rounded-2xl p-12 text-center">
          ...
          <p className="text-[#6b8f6b] text-sm">Cadastre propriedades, áreas e lotes para acessar os relatórios.</p>
          <Link href="/dashboard/lots/new" className="inline-flex items-center gap-2 bg-[#4caf50] hover:bg-[#43a047] text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors mt-5">
            Criar Lote
          </Link>
        </div>
      ) : (
        <div className="space-y-2">
          {lots.map((lot) => {
            const events = (lot as any).events ?? []
            const hasArea = !!lot.area_id
            const hasPlanting = events.some((e: any) => e.type === 'planting')
            const property = (lot as any).property

            const checksOk = [hasArea, hasPlanting, !!property?.car_number, !!property?.document_id]
            const score = checksOk.filter(Boolean).length
            const total = checksOk.length
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white font-semibold text-sm truncate">
                    {property?.name ?? '—'} — {CROP_LABELS[lot.crop_type] ?? lot.crop_type}
                    {lot.harvest_year ? ` (${lot.harvest_year})` : ''}
                  </p>
                  <p className="text-[#6b8f6b] text-xs mt-0.5">
                    {score}/{total} requisitos preenchidos · {events.length} evento{events.length !== 1 ? 's' : ''}
                  </p>
                </div>

                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-right">
                    <div className={`text-sm font-bold ${isGood ? 'text-green-400' : 'text-yellow-400'}`}>
