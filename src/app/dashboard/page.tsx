...
import { MapPin, Layers, Package, Activity, TrendingUp, ArrowRight } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  const [propertiesRes, lotsRes, eventsRes] = await Promise.all([
    supabase.from('properties').select('id, name', { count: 'exact' }),
    supabase.from('lots').select('id, crop_type, status', { count: 'exact' }),
    supabase
      .from('events')
      .select('*, lot:lots(property:properties(name))')
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div>
        ...
        <p className="text-[#6b8f6b] mt-1 text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Lotes por cultura */}
        <div className="lg:col-span-1 bg-[#172117] border border-[#2d3d2d] rounded-2xl p-5">
          ...
          {Object.keys(byCrop).length === 0 ? (
            ...
            <div className="space-y-3">
              {Object.entries(byCrop).map(([crop, count]) => (
                <div key={crop} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 bg-[#4caf50] rounded-full" style={{ width: `${(count / totalLots) * 80}px` }} />
        ...
        {/* EUDR Status */}
        <div className="lg:col-span-1 bg-gradient-to-br from-[#1a2e1a] to-[#172117] border border-[#2d3d2d] rounded-2xl p-5">
          <h3 className="text-white font-semibold text-sm mb-2">Conformidade EUDR</h3>
          <p className="text-[#6b8f6b] text-xs mb-4">
            Regulamento UE 2023/1115 — Desmatamento zero
          </p>

          <div className="space-y-3">
            {[
              { label: 'Propriedades georreferenciadas', done: totalProperties > 0 },
              { label: 'Lotes rastreados', done: totalLots > 0 },
              { label: 'Eventos de plantio registrados', done: recentEvents.some(e => e.type === 'planting') },
              ...
              <div key={i} className="flex items-center gap-2.5">
                ...
                <span className={`text-xs ${item.done ? 'text-[#a0b8a0]' : 'text-[#4d7a4d]'}`}>
                  {item.label}
                </span>
              </div>
            ))}
...
function MetricCard({
  ...
  return (
    <Link
      href={href}
      className="bg-[#172117] border border-[#2d3d2d] rounded-2xl p-5 hover:border-[#4caf50]/30 transition-colors group"
    >
